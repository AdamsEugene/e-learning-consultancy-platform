<!-- pages/dashboard/consultant/invoices/create.vue -->
...
// Page meta
useHead({
  title: "Create Invoice - Consultant Dashboard",
});
...
// Invoice items
const invoiceItems = ref<Partial<InvoiceItem>[]>([
  {
    id: 1,
    description: "",
    quantity: 1,
...
// Calculate due date (default to 14 days from issue date)
const calculateDueDate = () => {
  if (!invoiceData.value.issueDate) return;

  const dueDate = new Date(invoiceData.value.issueDate);
  dueDate.setDate(dueDate.getDate() + 14);
  invoiceData.value.dueDate = dueDate.toISOString().split("T")[0];
};

// Watch for changes to issue date to update due date
watch(
  () => invoiceData.value.issueDate,
  () => {
    calculateDueDate();
  }
);
...
const addInvoiceItem = () => {
  const newId =
    Math.max(0, ...invoiceItems.value.map((item) => item.id || 0)) + 1;
...
const saveAndSend = async () => {
  ...
  try {
    // In a real application, this would make an API call to save and send the invoice
...
<template>
  <div>
    ...
    <!-- Error Message -->
    <div
      v-if="errorMessage && formStatus !== 'success'"
      class="mb-6 bg-red-50 border border-red-200 text-red-800 rounded-lg p-4"
    >
      <div class="flex">
        <svg
          xmlns="http://www.w3.org/2000/svg"
    ...
    >
      <div class="flex">
        ...
        <span>Invoice saved successfully. Redirecting...</span>
      </div>
    </div>
    ...
    <div v-if="!showPreview">
      ...
      <div class="flex justify-end space-x-3">
        ...
        >
          <span v-if="formStatus !== 'saving'">Save as Draft</span>
        ...
        <button
          ...
          :disabled="formStatus === 'saving'"
        >
          <span v-if="formStatus !== 'saving'">Save & Send</span>
          <span v-else class="flex items-center">
            <svg
